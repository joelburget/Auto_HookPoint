...
class HookedTransformerConfig_From_AutoConfig(HookedTransformerConfig):
    ...
    def from_auto_config(
        cls, auto_config: PretrainedConfig, **kwargs
        ...
        return HookedTransformerConfig(
            ...
            d_head=auto_config.hidden_size
            // auto_config.num_attention_heads,  # NOTE will this work for all models?
            **kwargs,
        )
...
class HookedTransformerAdapterCfg:
    ...
    create_kwargs: Optional[
        Callable[[HookedTransformerConfig, Any], dict[str, Any]]
    ] = None
...
class AdaptedPosEmbed(PosEmbed):
    ...
    @classmethod
    def from_regular_pos_embed(
        cls, pos_embed: nn.Embedding, cfg: HookedTransformerConfig
    ):
        myclass = cls(cfg)
...
class HookedTransformerAdapter(HookedTransformer):
    ...
    def __init__(
        ...
        *,
        ...
        if isinstance(hf_model_name, str):
            self.model = auto_hook(
                AutoModelForCausalLM.from_pretrained(hf_model_name).to(
                    hooked_transformer_cfg.device
                )
            )
            ...
            raise ValueError(
                "Invalid input. Provide either a model name (str) or both model and tokenizer objects."
            )
    ...
    def apply_mappings(self):
        for ht_attr, model_attr in self.adapter_cfg.mappings.items():
            if model_attr is None:
                logging.warning(
                    f"No model attribute given for {ht_attr}, this might lead to errors"
                )
                continue
            ...
            if ht_attr == "blocks":
                ...
                if isinstance(value, nn.Embedding):
                    self.pos_embed = AdaptedPosEmbed.from_regular_pos_embed(
                        value, self.cfg
                    )
                else:
    ...
    def forward(
        ...
        with utils.LocallyOverridenDefaults(
            ...
            assert (
                len(blocks_and_idxs[start_at_layer:stop_at_layer]) > 0
            ), f"start_at_layer and stop_at_layer must be valid got start_at_layer={start_at_layer} and stop_at_layer={stop_at_layer}"
    ...
    def run_with_cache(self, *args, **kwargs):
        ...
        if isinstance(names_filter, str) and names_filter not in self.hook_dict.keys():
            ...
            raise ValueError(
                f"names_filter must be a key in hook_dict, got {names_filter}"
            )
    ...
    def validate_args(
        ...
        if (hf_model_name is not None) == (model is not None or tokenizer is not None):
            raise ValueError(
                "Provide either a model name or both model and tokenizer objects, not both or neither."
            )

    def check_blocks(self, blocks):
        assert isinstance(blocks, Iterable) and isinstance(
            blocks, Sized
        ), f"Expected an iterable of modules with __len__, got {type(blocks)}"
        assert (
            len(blocks) == self.cfg.n_layers
        ), f"Expected {self.cfg.n_layers} blocks, got {len(blocks)}"
        return list(blocks)
    ...
    def W_K(self) -> Float[torch.Tensor, "n_layers n_heads d_model d_head"]:
        raise NotImplementedError(
            "This method is not implemented for HookedTransformerAdapter"
        )
